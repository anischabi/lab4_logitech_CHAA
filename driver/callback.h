...
static void complete_callback(struct urb *urb) {
    ...
    if (urb->status == 0) {
        ...
        for (i = 0; i < urb->number_of_packets; ++i) {
            ...
            if (UrbPacketLength < 2 ||
                UrbPacketData[0] < 2 ||
                UrbPacketData[0] > UrbPacketLength)
                continue;

            /* -------- New frame (FID toggle) -------- */
            if (buffer->LastFID != (UrbPacketData[1] & STREAM_FID)) {
            ...
            /* -------- EOF? End frame immediately -------- */
            if (UrbPacketData[1] & STREAM_EOF) {
...
// this one looks like its working well
// static void complete_callback(struct urb *urb) {
//     struct driver_buffer  *buffer = urb->context;
//     unsigned char *UrbPacketData;
//     unsigned int   UrbPacketLength;
